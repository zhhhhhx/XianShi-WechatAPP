            ...
            if(result!=false){
                ...
                */
                ...
                for(let i=0;i<informationList.length;i++){ 
                    let j=i
                    if(informationList[j].type==1||informationList[j].type==-1){ //委托消息
                    ...
                    else if(informationList[j].type==2||informationList[j].type==-2){ //交易消息
                        let res=requestService.dbDocument('trade',informationList[j].trade_id,'获取消息相关信息成功')
                        res.then(function(result){
                        if(result!=false){
                            informationList[j]['title']=result.data.title, 
                            informationList[j]['receiver_name']=result.data.receiver_name,
